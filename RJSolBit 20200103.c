...
int solve (int p)
{
  ...
  for (; y < 27; ++y)        // Search Naked/Hidden Tuples Cell values Unit wise
  {
    ...
    if (Y > 4)
      continue;              // Skip Unit for > 4 solved Cell positions
    ...
    if (Y > 2)
      continue;              // Skip Unit for > 2 solved Cell positions
...
XYWT1Tf:
        ...
        if (B[g[K[0]]] < 3 || !(g[K[6]] & K[5]))
          continue;          // Skip for either XYZ-Hybrid Apex Cell values < three digits; or Wing Cells common value not in Hybrid Cell values
...
TEMP:
          ...
          if (b[g[K[0]]] < 3)
            continue;        // Skip for Apec Cell values < three digits
