...
int q,                       // Number of unsolved Cell positions Grid wise
    r[81],                   // Used for sorting and removing each unsolved Cell position Grid wise
...
int solve (int p)
{
  ...
  for (y = 0, a = p; a < q; ++a)
    ...
    if (B[z] > B[g[r[a]]])   // Check least Cell values in current unsolved Cell position
      if (B[z = g[r[x = a]]] < 2)
        goto NHSCF;          // Found Naked single Cell value in current unsolved Cell position
  for (; y < 27; ++y)        // Search Naked/Hidden Tuples Cell values Unit wise
  {
    Y = !g[l[y][0]] + !g[l[y][1]] + !g[l[y][2]] + !g[l[y][3]] +
      !g[l[y][4]] + !g[l[y][5]] + !g[l[y][6]] + !g[l[y][7]] + !g[l[y][8]];
                             // Count Unit not unsolved Cell positions
    if (Y > 6)
      continue;              // Skip Unit for not unsolved Cell positions > 6
    for (a = 0; a < 36; ++a) // Search Naked/Hidden pair Cell values Unit 36 pair Cell positions wise
    {
      ...
      printf ("%d) Hidden pair: %s %d %d @ %s %s =>",
        p, RCB, LBN (l[y][h[a][0]]), b[K[0]], S[l[y][h[a][0]]], S[l[y][h[a][1]]]);
      if (k[0] ^ (k[0] & K[0]))
        printf (" -%d @ %s", b[k[0] ^ (k[0] & K[0])], S[l[y][h[a][0]]]);
      if (k[1] ^ (k[1] & K[0]))
        printf (" -%d @ %s", b[k[1] ^ (k[1] & K[0])], S[l[y][h[a][1]]]);
      printf ("\n");
#endif
      if (solve (p))
        return 1;
    ...
    if (Y > 4)
      continue;              // Skip Unit for not unsolved Cell positions > 4
    for (; a < 120; ++a)     // Search Naked/Hidden triplet Cell values Unit 84 triplet Cell positions wise
    {
      ...
      if (k[1] ^ (k[1] & K[0]))
        printf (" -%d @ %s", b[k[1] ^ (k[1] & K[0])], S[l[y][h[a][1]]]);
      if (k[2] ^ (k[2] & K[0]))
        printf (" -%d @ %s", b[k[2] ^ (k[2] & K[0])], S[l[y][h[a][2]]]);
      printf ("\n");
#endif
      if (solve (p))
        return 1;
    ...
    for (; a < 246; ++a)     // Search Naked/Hidden quad Cell values Unit 126 quad Cell positions wise
    {
      ...
      if (k[2] ^ (k[2] & K[0]))
        printf (" -%d @ %s", b[k[2] ^ (k[2] & K[0])], S[l[y][h[a][2]]]);
      if (k[3] ^ (k[3] & K[0]))
        printf (" -%d @ %s", b[k[3] ^ (k[3] & K[0])], S[l[y][h[a][3]]]);
      printf ("\n");
#endif
      if (solve (p))
        return 1;
  ...
  for (a = 0; a < 54; ++a)   // Search Locked candidate for 54 mini-Lines 3 Cell positions
      ...
      {                      // Locked candidate in Box other Cell positions
        int k[6] = {g[j[a][Z ? 3 : 9]], g[j[a][Z ? 4 : 10]], g[j[a][Z ? 5 : 11]],
                   g[j[a][Z ? 6 : 12]], g[j[a][Z ? 7 : 13]], g[j[a][Z ? 8 : 14]]};
                             // Backup and drop Locked candidate from Line / Box other Cell positions
        g[j[a][Z ? 3 : 9]] &= ~y;
        g[j[a][Z ? 4 : 10]] &= ~y;
        g[j[a][Z ? 5 : 11]] &= ~y;
        g[j[a][Z ? 6 : 12]] &= ~y;
  ...
  for (a = 0; a < 54; ++a)   // Search Almost Locked Pair, Triple and Quad 54 mini-Lines 3 Cell positions wise
  {
    ...
    for (y = 0; y < 2; ++y)  // Search either away Line or away Box 6 Cell position wise
    {
      
    }

    for (X = 3; X < 9; ++X)  // Search Almost Locked Pair 54 mini-Lines away Line 6 Cell positions wise
    {
      if (B[Y = g[j[a][X]] & ~(g[j[a][3 + (X < 4)]] |
        g[j[a][4 + (X < 5)]] | g[j[a][5 + (X < 6)]] |
        g[j[a][6 + (X < 7)]] | g[j[a][7 + (X < 8)]])] == 2 &&
        (g[j[a][Z = 9]] == Y || g[j[a][Z = 10]] == Y ||
        g[j[a][Z = 11]] == Y || g[j[a][Z = 12]] == Y ||
        g[j[a][Z = 13]] == Y || g[j[a][Z = 14]] == Y) &&
        (B[g[j[a][X]]] > 2 || (g[j[a][9 + (Z < 10)]] |
        g[j[a][10 + (Z < 11)]] |  g[j[a][11 + (Z < 12)]] |
        g[j[a][12 + (Z < 13)]] | g[j[a][13 + (Z < 14)]]) & Y))
      {                      // Found Almost Locked Pair Line wise
        int k[7] = {g[j[a][X]], g[j[a][9]], g[j[a][10]], g[j[a][11]],
                   g[j[a][12]], g[j[a][13]], g[j[a][14]]};
        ...
        printf ("%d) Almost Locked Pair: %s %d Box %d wise %d @ r%dc%d %s b%d%s\n=> -%d @ %s %s %s %s %s",
          p, a < 27 ? "Row" : "Column",
          a < 27 ? ROW (w[j[a][X]][20]) : COL (w[j[a][X]][20]), BOX (j[a][Z]),
          b[Y], ROW (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]),
          COL (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]), S[j[a][X]],
          BOX (j[a][Z]), S[j[a][Z]], b[Y], S[j[a][9 + (Z < 10)]],
          S[j[a][10 + (Z < 11)]], S[j[a][11 + (Z < 12)]],
          S[j[a][12 + (Z < 13)]], S[j[a][13 + (Z < 14)]]);
        if (k[0] ^ Y)
          printf (" => -%d @ %s", b[k[0] ^ Y], S[j[a][X]]);
        printf ("\n");
#endif
        if (solve (p))
          return 1;
    ...
    for (X = 9; X < 15; ++X) // Search Almost Locked Pair 54 mini-Lines away Box 6 Cell positions wise
    {
      if (B[Y = g[j[a][X]] & ~(g[j[a][9 + (X < 10)]] |
        g[j[a][10 + (X < 11)]] | g[j[a][11 + (X < 12)]] |
        g[j[a][12 + (X < 13)]] | g[j[a][13 + (X < 14)]])] == 2 &&
        (g[j[a][Z = 3]] == Y || g[j[a][Z = 4]] == Y ||
        g[j[a][Z = 5]] == Y || g[j[a][Z = 6]] == Y ||
        g[j[a][Z = 7]] == Y || g[j[a][Z = 8]] == Y) &&
        (B[g[j[a][X]]] > 2 || (g[j[a][3 + (Z < 4)]] |
        g[j[a][4 + (Z < 5)]] | g[j[a][5 + (Z < 6)]] |
        g[j[a][6 + (Z < 7)]] | g[j[a][7 + (Z < 8)]]) & Y))
      {                      // Found Almost Locked Pair Box wise
        int k[7] = {g[j[a][X]], g[j[a][3]], g[j[a][4]], g[j[a][5]],
                   g[j[a][6]], g[j[a][7]], g[j[a][8]]};
                             // Backup and drop Almost Locked Pair from removal Cell positions
        g[j[a][X]] &= Y;
        g[j[a][3]] &= ~Y;
        g[j[a][4]] &= ~Y;
        g[j[a][5]] &= ~Y;
        ...
        g[j[a][Z]] = Y;
#if RJ > 2
        printf ("%d) Almost Locked Pair: Box %d %s %d wise %d @ r%dc%d b%d%s %s\n=> -%d @ %s %s %s %s %s",
          p, BOX (j[a][X]), a < 27 ? "Row" : "Column",
          a < 27 ? ROW (w[j[a][Z]][20]) : COL (w[j[a][Z]][20]), b[Y],
          ROW (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]),
          COL (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]), BOX (j[a][X]),
          S[j[a][X]], S[j[a][Z]], b[Y], S[j[a][3 + (Z < 4)]],
          S[j[a][4 + (Z < 5)]], S[j[a][5 + (Z < 6)]],
          S[j[a][6 + (Z < 7)]], S[j[a][7 + (Z < 8)]]);
        if (k[0] ^ Y)
          printf (" => -%d @ %s", b[k[0] ^ Y], S[j[a][X]]);
        printf ("\n");
#endif
        if (solve (p))
          return 1;
#if RJ > 2
        printf ("%d) Undo Almost Locked Pair: Box %d %s %d wise %d @ r%dc%d b%d%s %s\n<= +%d @ %s %s %s %s %s",
          p, BOX (j[a][X]), a < 27 ? "Row" : "Column",
          a < 27 ? ROW (w[j[a][Z]][20]) : COL (w[j[a][Z]][20]), b[Y],
          ROW (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]),
          COL (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]), BOX (j[a][X]),
    ...
    for (y = 3; y < 8; ++y)
    {                        // Search Almost Locked Triple 1st Cell position away Line wise
      if (!g[j[a][y]])
        continue;            // Skip for 1st Cell position not unsolved
      for (M = y + 1; M < 9; ++M)
      {                      // Search 2nd Cell position away Line wise
        if (!g[j[a][M]] || B[Y = (g[j[a][y]] | g[j[a][M]]) &
          ~(g[j[a][3 + (y < 4) + (M < 5)]] |
          g[j[a][4 + (y < 5) + (M < 6)]] |
          g[j[a][5 + (y < 6) + (M < 7)]] |
          g[j[a][6 + (y < 7) + (M < 8)]])] != 3)
        ...
        for (N = 9; N < 14; ++N)
        {                    // Search 3rd Cell position away Box wise
          if (!g[j[a][N]] || B[g[j[a][N]]] > 3)
          ...
          {                  // Search 4th Cell position away Box wise
            if (!g[j[a][Z]] || (g[j[a][N]] | g[j[a][Z]]) != Y ||
                             // Skip for either 4th Cell position not unsolved; or 3rd and 4th Cell values not 1st and 2nd Cell values; or
              (B[g[j[a][y]] | g[j[a][M]]] < 4 &&
              !((g[j[a][9 + (N < 10) + (Z < 11)]] |
              g[j[a][10 + (N < 11) + (Z < 12)]] |
              g[j[a][11 + (N < 12) + (Z < 13)]] |
              g[j[a][12 + (N < 13) + (Z < 14)]]) & Y)))
              continue;      // Almost Locked Triple values not in removal Cell positions
            int k[6] = {g[j[a][y]], g[j[a][M]],
                       g[j[a][9 + (N < 10) + (Z < 11)]],
                       g[j[a][10 + (N < 11) + (Z < 12)]],
                       g[j[a][11 + (N < 12) + (Z < 13)]],
                       g[j[a][12 + (N < 13) + (Z < 14)]]};
                             // Backup and drop Almost Locked Triple values from removal Cell positions
            g[j[a][y]] &= Y;
            g[j[a][M]] &= Y;
            g[j[a][9 + (N < 10) + (Z < 11)]] &= ~Y;
            g[j[a][10 + (N < 11) + (Z < 12)]] &= ~Y;
            g[j[a][11 + (N < 12) + (Z < 13)]] &= ~Y;
            g[j[a][12 + (N < 13) + (Z < 14)]] &= ~Y;
#if RJ > 2
            printf ("%d) Almost Locked Triple: %s %d Box %d wise %d @ r%dc%d r%dc%d b%d%s b%d%s\n=> -%d @ %s %s %s %s",
              p, a < 27 ? "Row" : "Column",
              a < 27 ? ROW (w[j[a][y]][20]) : COL (w[j[a][y]][20]), BOX (j[a][N]),
              b[Y], ROW (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]),
              COL (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]),
              ROW (w[j[a][y]][20] | w[j[a][M]][20]),
              COL (w[j[a][y]][20] | w[j[a][M]][20]),
              BOX (j[a][N]), S[j[a][N]], BOX (j[a][Z]), S[j[a][Z]],
              b[Y], S[j[a][9 + (N < 10) + (Z < 11)]],
              S[j[a][10 + (N < 11) + (Z < 12)]],
              S[j[a][11 + (N < 12) + (Z < 13)]],
              S[j[a][12 + (N < 13) + (Z < 14)]]);
            if (k[0] ^ (k[0] & Y))
              printf (" => -%d @ %s", b[k[0] ^ (k[0] & Y)], S[j[a][y]]);
            if (k[1] ^ (k[1] & Y))
              printf (" => -%d @ %s", b[k[1] ^ (k[1] & Y)], S[j[a][M]]);
            printf ("\n");
#endif
              if (solve (p))
                return 1;
            ...
            g[j[a][y]] = k[0];
            g[j[a][M]] = k[1];
            g[j[a][9 + (N < 10) + (Z < 11)]] = k[2];
            g[j[a][10 + (N < 11) + (Z < 12)]] = k[3];
            g[j[a][11 + (N < 12) + (Z < 13)]] = k[4];
            g[j[a][12 + (N < 13) + (Z < 14)]] = k[5];
#if RJ > 3
            prn ();
#endif
            return 0;        // Undo Almost Locked Triple values to removal Cell positions
          }
        }
      }
    }
    for (L = 9; L < 14; ++L)
    {                        // Search Almost Locked Triple 1st Cell position away Box wise
      if (!g[j[a][L]])
        continue;            // Skip for 1st Cell position not unsolved
      for (M = L + 1; M < 15; ++M)
      {                      // Search 2nd Cell position away Box wise
        ...
        {                    // Search 3rd Cell position away Line wise
          if (!g[j[a][N]] || B[g[j[a][N]]] > 3)
            continue;        // Skip for either 3rd Cell position not unsolved; or 3rd Cell values > 3 digits
          for (Z = N + 1; Z < 9; ++Z)
          {                  // Search 4th Cell position away Line wise
            if (!g[j[a][Z]] || (g[j[a][N]] | g[j[a][Z]]) != Y ||
              ...
              (B[g[j[a][L]] | g[j[a][M]]] < 4 &&
              !((g[j[a][3 + (N < 4) + (Z < 5)]] |
              g[j[a][4 + (N < 5) + (Z < 6)]] | g[j[a][5 + (N < 6) + (Z < 7)]] |
              g[j[a][6 + (N < 7) + (Z < 8)]]) & Y)))
              continue;      // Almost Locked Triple values not in removal Cell positions
            ...
            g[j[a][6 + (N < 7) + (Z < 8)]] &= ~Y;
#if RJ > 2
            printf ("%d) Almost Locked Triple: Box %d %s %d wise %d @ r%dc%d b%d%s b%d%s r%dc%d\n=> -%d @ %s %s %s %s",
              p, BOX (j[a][L]), a < 27 ? "Row" : "Column",
              a < 27 ? ROW (w[j[a][N]][20]) : COL (w[j[a][N]][20]), b[Y],
              ROW (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]),
              COL (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]),
              BOX (j[a][L]), S[j[a][L]], BOX (j[a][M]), S[j[a][M]],
              ROW (w[j[a][N]][20] | w[j[a][Z]][20]),
              COL (w[j[a][N]][20] | w[j[a][Z]][20]),
              b[Y], S[j[a][3 + (N < 4) + (Z < 5)]],
              S[j[a][4 + (N < 5) + (Z < 6)]], S[j[a][5 + (N < 6) + (Z < 7)]],
              S[j[a][6 + (N < 7) + (Z < 8)]]);
            if (k[0] ^ (k[0] & Y))
              printf (" => -%d @ %s", b[k[0] ^ (k[0] & Y)], S[j[a][L]]);
            if (k[1] ^ (k[1] & Y))
              printf (" => -%d @ %s", b[k[1] ^ (k[1] & Y)], S[j[a][M]]);
            printf ("\n");
#endif
            if (solve (p))
              return 1;
#if RJ > 2
            printf ("%d) Undo Almost Locked Triple: Box %d %s %d wise %d @ r%dc%d b%d%s b%d%s r%dc%d\n<= +%d @ %s %s %s %s",
              p, BOX (j[a][L]), a < 27 ? "Row" : "Column",
              a < 27 ? ROW (w[j[a][N]][20]) : COL (w[j[a][N]][20]), b[Y],
              ROW (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]),
              COL (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]),
    ...
    if (!g[j[a][3]] + !g[j[a][4]] + !g[j[a][5]] +
      ...
      continue;              // Skip Almost Locked Quad for either > three mini-Line away Line not unsolved Cell positions; or > three mini-Line away Box not unsolved Cell positions
    for (A = 3; A < 7; ++A)
    {                        // Search Almost Locked Quad 1st Cell position 54 mini-Lines away Line wise
      if (!g[j[a][A]])
        continue;            // Skip for 1st Cell position not unsolved
      for (L = A + 1; L < 8; ++L)
      {
        ...
        for (M = L + 1; M < 9; ++M)
        {
          ...
          {
            if (!g[j[a][N]])
              continue;      // Skip for 4th Cell position not unsolved
            for (X = N + 1; X < 14; ++X)
            {
              if (!g[j[a][X]])
                continue;    // Skip for 5th Cell position not unsolved
              for (Z = X + 1; Z < 15; ++Z)
              {
                if (!g[j[a][Z]] || (g[j[a][N]] | g[j[a][X]] | g[j[a][Z]]) != Y ||
                  ...
                  !((g[j[a][9 + (N < 10) + (X < 11) + (Z < 12)]] |
                  g[j[a][10 + (N < 11) + (X < 12) + (Z < 13)]] |
                  g[j[a][11 + (N < 12) + (X < 13) + (Z < 14)]]) & Y)))
                  continue;  // Almost Locked Quad values not in removal Cell positions
                int k[6] = {g[j[a][A]], g[j[a][L]], g[j[a][M]],
                           g[j[a][9 + (N < 10) + (X < 11) + (Z < 12)]],
                           g[j[a][10 + (N < 11) + (X < 12) + (Z < 13)]],
                           g[j[a][11 + (N < 12) + (X < 13) + (Z < 14)]]};
                             // Backup and drop Almost Locked Quad values from removal Cell positions
                g[j[a][A]] &= Y;
                g[j[a][L]] &= Y;
                g[j[a][M]] &= Y;
                g[j[a][9 + (N < 10) + (X < 11) + (Z < 12)]] &= ~Y;
                g[j[a][10 + (N < 11) + (X < 12) + (Z < 13)]] &= ~Y;
                g[j[a][11 + (N < 12) + (X < 13) + (Z < 14)]] &= ~Y;
#if RJ > 2
                printf ("%d) Almost Locked Quad: %s %d Box %d wise %d @ r%dc%d r%dc%d b%d%s b%d%s b%d%s\n=> -%d @ %s %s %s",
                  p, a < 27 ? "Row" : "Column",
                  a < 27 ? ROW (w[j[a][A]][20]) : COL (w[j[a][A]][20]), BOX (j[a][N]),
                  b[Y], ROW (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]),
                  COL (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]),
                  ...
                  S[j[a][9 + (N < 10) + (X < 11) + (Z < 12)]],
                  S[j[a][10 + (N < 11) + (X < 12) + (Z < 13)]],
                  S[j[a][11 + (N < 12) + (X < 13) + (Z < 14)]]);
                if (k[0] ^ (k[0] & Y))
                  printf (" => -%d @ %s", b[k[0] ^ (k[0] & Y)], S[j[a][A]]);
                if (k[1] ^ (k[1] & Y))
                  printf (" => -%d @ %s", b[k[1] ^ (k[1] & Y)], S[j[a][L]]);
                if (k[2] ^ (k[2] & Y))
                  printf (" => -%d @ %s", b[k[2] ^ (k[2] & Y)], S[j[a][M]]);
                printf ("\n");
#endif
                if (solve (p))
                  return 1;
                ...
                g[j[a][L]] = k[1];
                g[j[a][M]] = k[2];
                g[j[a][9 + (N < 10) + (X < 11) + (Z < 12)]] = k[3];
                g[j[a][10 + (N < 11) + (X < 12) + (Z < 13)]] = k[4];
                g[j[a][11 + (N < 12) + (X < 13) + (Z < 14)]] = k[5];
#if RJ > 3
                prn ();
#endif
                return 0;    // Undo Almost Locked Quad values to removal Cell positions
              }
            }
          }
        }
    ...
    for (A = 9; A < 13; ++A)
    {                        // Search Almost Locked Quad 1st Cell position 54 mini-Lines away Box wise
      if (!g[j[a][A]])
        continue;            // Skip for 1st Cell position not unsolved
      for (L = A + 1; L < 14; ++L)
      {
        ...
        {
          if (!g[j[a][M]] || B[Y = (g[j[a][A]] | g[j[a][L]] | g[j[a][M]]) &
            ~(g[j[a][9 + (A < 10) + (L < 11) + (M < 12)]] |
            g[j[a][10 + (A < 11) + (L < 12) + (M < 13)]] |
            g[j[a][11 + (A < 12) + (L < 13) + (M < 14)]])] != 4 ||
            !(g[j[a][A]] & Y) || !(g[j[a][L]] & Y) || !(g[j[a][M]] & Y))
            continue;        // Skip for either 3rd Cell position not unsolved; or 1st, 2nd and 3rd Cells values not four digits
          for (N = 3; N < 7; ++N)
          {
            ...
            for (X = N + 1; X < 8; ++X)
            {
              ...
              for (Z = X + 1; Z < 9; ++Z)
              {
                ...
                g[j[a][L]] &= Y;
                g[j[a][M]] &= Y;
                g[j[a][3 + (N < 4) + (X < 5) + (Z < 6)]] &= ~Y;
                g[j[a][4 + (N < 5) + (X < 6) + (Z < 7)]] &= ~Y;
                g[j[a][5 + (N < 6) + (X < 7) + (Z < 8)]] &= ~Y;
#if RJ > 2
                printf ("%d) Almost Locked Quad: %s %d Box %d wise %d @ r%dc%d r%dc%d b%d%s b%d%s b%d%s\n=> -%d @ %s %s %s",
                  p, a < 27 ? "Row" : "Column",
                ...
                g[j[a][L]] = k[1];
                g[j[a][M]] = k[2];
                g[j[a][3 + (N < 4) + (X < 5) + (Z < 6)]] = k[3];
                g[j[a][4 + (N < 5) + (X < 6) + (Z < 7)]] = k[4];
                g[j[a][5 + (N < 6) + (X < 7) + (Z < 8)]] = k[5];
#if RJ > 3
                prn ();
#endif
                return 0;    // Undo Almost Locked Quad values to removal Cell positions
        ...
        {                    // Search 2nd SL within 2nd and 3rd Chutes
          ...
          {                  // Found Skyscraper and Grouped Skyscraper digit in removal Cell positions
            ...
            printf ("%d)%sSkyscraper: %ss %d %d Base Cells %d @ %s %s Cover Cells %d @ r%dc%d r%dc%d => -%d @",
              p, A[5] ? " Grouped " : " ", RCB, a - y + 1, Z - y + 1, b[Y], S[K[4]], S[K[5]],
            ...
            if (solve (p))
              return 1;
#if RJ > 2
            printf ("%d) Undo%sSkyscraper: %ss %d %d Base Cells %d @ %s %s Cover Cells %d @ r%dc%d r%dc%d => -%d @",
              p, A[5] ? " Grouped " : " ", RCB, a - y + 1, Z - y + 1, b[Y], S[K[4]], S[K[5]],
              b[Y], ROW ((K[6] + 1 ? w[K[6]][20] : 0) | (K[7] + 1 ? w[K[7]][20] : 0) |
              (K[8] + 1 ? w[K[8]][20] : 0)), COL ((K[6] + 1 ? w[K[6]][20] : 0) |
              (K[7] + 1 ? w[K[7]][20] : 0) | (K[8] + 1 ? w[K[8]][20] : 0)),
            ...
            prn ();
#endif
            return 0;        // Undo Skyscraper and Grouped Skyscraper digit to removal Cell positions
          }
        }
      }
  for (a = 0; a < 81; ++a)   // Search 2-String Kite and Grouped 2-String Kite with ERI Ring, ERI and Box Grid wise
  {
    ...
    {                        // Search 2-String Kite and Grouped 2-String Kite with ERI Ring and ERI digit wise
      if (B[X = !(g[w[a][0]] & Y) | (!(g[w[a][1]] & Y) << 1) |
        (!(g[w[a][2]] & Y) << 2) | (!(g[w[a][3]] & Y) << 3) |
        (!(g[w[a][4]] & Y) << 4) | (!(g[w[a][5]] & Y) << 5)] != 5 ||
        B[Z = !(g[w[a][14]] & Y) | (!(g[w[a][15]] & Y) << 1) |
        (!(g[w[a][16]] & Y) << 2) | (!(g[w[a][17]] & Y) << 3) |
        (!(g[w[a][18]] & Y) << 4) | (!(g[w[a][19]] & Y) << 5)] != 5)
      ...
      if ((g[L = w[K[3]][b[~Z & 63] > 3 ? 14 : 17]] | g[w[L][12]] | g[w[L][13]] |
        g[M = w[K[4]][b[~X & 63] > 3 ? 0 : 3]] | g[w[M][6]] | g[w[M][7]] |
      ...
      {                      // Check 2-String Kite+ERI and Grouped 2-String Kite+ERI
        ...
        g[K[5] = w[K[5]][K[2] = 7]] -= Y;
                             ...
                             // Check 2-String Kite+Box and Grouped 2-String Kite+Box
        g[K[5] = w[K[5]][K[2] = 12]] -= Y;
      else if (LB3 (M, w[M][13], w[M][12], 0) & g[w[K[5]][13]] & Y)
                             // Check 2-String Kite+Box and Grouped 2-String Kite+Box
        g[K[5] = w[K[5]][K[2] = 13]] -= Y;
      else
        continue;
#if RJ > 2
      printf ("%d)%s2-String Kite%s SL Row %d between Cell %d @ ", p,
      ...
      if (solve (p))
        return 1;
#if RJ > 2
      printf ("%d) Undo%s2-String Kite%s SL Row %d between Cell %d @ ", p,
        ((g[w[a][6]] & g[w[a][7]]) | (g[w[a][12]] & g[w[a][13]])) & Y ? " Grouped " : " ",
        K[0] + 1 ? "+ERI Ring:" : (K[1] + 1 ? "+ERI:" : (K[2] + 1 ? "+Box:" : ":")),
        ROW (w[a][20]), b[Y]);
      if (g[w[a][6]] & g[w[a][7]] & Y)
      ...
      else                   // 2-String Kite, 2-String Kite+Box, Grouped 2-String Kite and Grouped 2-String Kite+Box
        g[K[5]] += Y;
#if RJ > 3
      prn ();
#endif
      return 0;              // Undo 2-String Kite and Grouped 2-String Kite with ERI Ring, ERI and Box digit to removal Cell position
    }
  }
    ...
    for (; K[1] = K[0] & -K[0]; K[0] -= K[1])
      ...
      for (y = 0; y < 2; ++y)// Search SL Line wise
        ...
        {                    // Search SL 1st Cell position Line wise
          ...
          {                  // No ERI digit in six Line Cell positions
            ...
            {                // Dual Linked Empty Rectangle removals
              int k[15] = {g[K[7]], g[w[K[7]][K[5]]], g[w[K[7]][K[5] + 1]], g[w[K[7]][K[5] + 2]],
                          ...
                          g[w[K[6]][W[3][!y]]], g[w[K[6]][W[4][!y]]]};
                             // Backup Dual Linked Empty Rectangle removal Cells values and drop ERI digit
              g[K[7]] &= ~K[1];
              g[w[K[7]][K[5]]] &= ~K[1];
              g[w[K[7]][K[5] + 1]] &= ~K[1];
              g[w[K[7]][K[5] + 2]] &= ~K[1];
              ...
              printf ("%d) Dual Linked Empty Rectangle: ERI %d @ b%d%s and ERI %d @ b%d%s SL %s %d between Cell %d @ %s and Cell %d @ %s\n=> -%d @ r%dc%d r%dc%d\n",
                p, b[K[1]], BOX (a), S[a], b[K[1]], BOX (K[7]), S[K[7]], R_C, RCN (K[2]),
              ...
              if (solve (p))
                return 1;
#if RJ > 2
              printf ("%d) Undo Dual Linked Empty Rectangle: ERI %d @ b%d%s and ERI %d @ b%d%s SL %s %d between Cell %d @ %s and Cell %d @ %s\n<= +%d @ r%dc%d r%dc%d\n",
                p, b[K[1]], BOX (a), S[a], b[K[1]], BOX (K[7]), S[K[7]], R_C, RCN (K[2]),
                b[K[1]], S[K[2]], b[K[1]], S[w[K[2]][W[1][y] + b[~K[3] & 63] - 1]],
                b[K[1]], ROW (w[K[7]][20] | w[w[K[7]][K[5]]][20] |
                w[w[K[7]][K[5] + 1]][20] | w[w[K[7]][K[5] + 2]][20] |
              ...
              prn ();
#endif
              return 0;      // Undo Dual Linked Empty Rectangle removal Cell values
            }
            if (!(g[K[7]] & K[1]))
            ...
            printf ("%d) Empty Rectangle: ERI %d @ b%d%s SL %s %d between Cell %d @ %s and Cell %d @ %s => -%d @ %s\n",
              p, b[K[1]], BOX (a), S[a], R_C, RCN (K[2]), b[K[1]], S[K[2]],
              b[K[1]], S[w[K[2]][W[1][y] + b[~K[3] & 63] - 1]], b[K[1]], S[K[7]]);
            if (!(g[w[K[7]][W[3][!y]]] & K[1]) + !(g[w[K[7]][W[4][!y]]] & K[1]) +
              !(g[w[K[7]][W[1][!y]]] & K[1]) + !(g[w[K[7]][W[18][!y]]] & K[1]) +
              !(g[w[K[7]][W[19][!y]]] & K[1]) + !(g[w[K[7]][W[20][!y]]] & K[1]) +
              !(g[w[K[7]][W[21][!y]]] & K[1]) + !(g[w[K[7]][W[22][!y]]] & K[1]) == 7)
            {                // Check one ERI digit in 2nd Line Cell positions
              g[K[2]] -= K[1];
              printf ("%d) Empty Rectangle: ERI %d @ b%d%s SL %s %d between Cell %d @ %s and Cell %d @ %s => -%d @ %s\n",
                p, b[K[1]], BOX (a), S[a], y ? "Row" : "Column",
            ...
            if (solve (p))
              return 1;
#if RJ > 2
            printf ("%d) Undo Empty Rectangle: ERI %d @ b%d%s SL %s %d between Cell %d @ %s and Cell %d @ %s <= +%d @ %s\n",
              p, b[K[1]], BOX (a), S[a], R_C, RCN (K[2]), b[K[1]], S[K[2]],
              b[K[1]], S[w[K[2]][W[1][y] + b[~K[3] & 63] - 1]], b[K[1]], S[K[7]]);
            if (!(g[w[K[7]][W[3][!y]]] & K[1]) + !(g[w[K[7]][W[4][!y]]] & K[1]) +
              !(g[w[K[7]][W[1][!y]]] & K[1]) + !(g[w[K[7]][W[18][!y]]] & K[1]) +
          ...
          for (K[3] = W[1][y]; K[3] < W[2][y]; ++K[3])
            if (ERI (w[K[2]][K[3]]) & K[1])
              break;         // Check ERI digit in SL 2nd ERI Cell position
          if (K[3] == W[2][y])
          ...
          {                  // Grouped Linked Empty Rectangle Intersection removal Cell positions
            int k[16] = {g[a], g[w[a][K[5]]], g[w[a][K[5] + 1]], g[w[a][K[5] + 2]],
                        ...
                        g[K[7]], g[w[a][K[6]]], g[w[a][K[6] + 1]], g[w[a][K[6] + 2]]};
                             // Backup Grouped Linked Empty Rectangle removal Cells values and drop ERI digit
            g[a] &= ~K[1];
            g[w[a][K[5]]] &= ~K[1];
            g[w[a][K[5] + 1]] &= ~K[1];
            g[w[a][K[5] + 2]] &= ~K[1];
            ...
            if (solve (p))
              return 1;
#if RJ > 2
            printf ("%d) Undo Grouped Linked Empty Rectangle: ERI %d @ b%d%s and ERI %d @ b%d%s SL %s %d between ERI %d @ b%d%s and ERI %d @ b%d%s\n<= +%d @ r%dc%d r%dc%d\n",
              p, b[K[1]], BOX (a), S[a], b[K[1]], BOX (K[7]), S[K[7]], R_C, RCN (K[2]),
              b[K[1]], BOX (K[2]), S[K[2]], b[K[1]], BOX (K[4]), S[K[4]], b[K[1]],
              ROW (w[a][20] | w[K[7]][20] | w[w[a][K[6]]][20] | w[w[a][K[6] + 1]][20] |
              w[w[a][K[6] + 2]][20] | w[K[2]][20] | w[K[4]][20] | w[w[K[2]][K[6]]][20] |
            ...
            prn ();
#endif
            return 0;        // Undo Grouped Linked Empty Rectangle removal Cell positions
          }
          if (!(g[K[7]] & K[1]))
          ...
          printf ("%d) Grouped Empty Rectangle: ERI %d @ b%d%s SL %s %d between ERI %d @ b%d%s and ERI %d @ b%d%s => -%d @ %s\n",
            p, b[K[1]], BOX (a), S[a], R_C, RCN (K[2]), b[K[1]], BOX (K[2]), S[K[2]],
            b[K[1]], BOX (K[4]), S[K[4]], b[K[1]], S[K[7]]);
#endif
          if (solve (p))
            return 1;
#if RJ > 2
          printf ("%d) Undo Grouped Empty Rectangle: ERI %d @ b%d%s SL %s %d between ERI %d 2 b%d%s and ERI %d @ b%d%s <= +%d @ %s\n",
            p, b[K[1]], BOX (a), S[a], R_C, RCN (K[2]), b[K[1]], BOX (K[2]), S[K[2]],
            b[K[1]], BOX (K[4]), S[K[4]], b[K[1]], S[K[7]]);
#endif
          g[K[7]] |= K[1];
...
/**
  for (a = p; a < q; ++a)    // Search M-Wings and M-Rings unsolved Cell position wise
  {
    if (B[g[r[a]]] > 2)
      continue;              // Skip for unsolved Cell values > 2 digits
    int K[10] = {r[a]};      // Assign bivalue Cell position

    for (y = 0; y < 2; ++y)  // Search 1st SL Line wise
    {
      for (K[1] = W[1][y]; K[1] < W[2][y]; ++K[1])
      {                      // M-Wing Type 1A, 1B, 2A, 2B, 2C, 2D, 3A, 3B, 4A, 4B
        if (!B[K[2] = g[K[3] = w[K[0]][K[1]]] & g[K[0]]] ||
                             // Skip for either no bivalue Cell value in 1st SL Cell values; or
          ((g[w[K[3]][W[3][!y]]] | g[w[K[3]][W[4][!y]]]) & g[K[0]]) ||
                             // bivalue Cell values in Box-Line Cell values; or
          B[K[4] = (g[w[K[3]][W[1][!y]]] & g[K[0]] ? 1 : 0) +
          (g[w[K[3]][W[18][!y]]] & g[K[0]] ? 2 : 0) +
          (g[w[K[3]][W[19][!y]]] & g[K[0]] ? 4 : 0) +
          (g[w[K[3]][W[20][!y]]] & g[K[0]] ? 8 : 0) +
          (g[w[K[3]][W[21][!y]]] & g[K[0]] ? 16 : 0) +
          (g[w[K[3]][W[22][!y]]] & g[K[0]] ? 32 : 0)] != 1 ||
                             // no bivalue Cell values in Line another 1st SL Cell positions; or
          B[g[K[5] = w[K[3]][W[1][!y] + b[K[4]] - 1]] & g[K[0]]] != 2)
          continue;          // no bivalue Cell values found in Line another 1st SL Cell values
        K[6] = g[K[0]] - (B[g[K[0]] & K[2]] > 1 ? 0 : K[2]);
        if ((Y = B[K[7] = (g[w[K[5]][W[17][!y]]] & K[6] ? 1 : 0) +
          (g[w[K[5]][W[17][!y] + 1]] & K[6] ? 2 : 0) +
          (g[w[K[5]][W[17][!y] + 2]] & K[6] ? 4 : 0) +
          (g[w[K[5]][W[17][!y] + 3]] & K[6] ? 8 : 0) +
          (g[w[K[5]][W[17][!y] + 4]] & K[6] ? 16 : 0) +
          (g[w[K[5]][W[17][!y] + 5]] & K[6] ? 32 : 0) +
          (g[w[K[5]][W[17][!y] + 6]] & K[6] ? 64 : 0) +
          (g[w[K[5]][W[17][!y] + 7]] & K[6] ? 128 : 0)] == 1 &&
          (g[K[8] = K[0] + (K[9] = w[K[5]][W[17][!y] + b[K[7]] - 1])] & K[6])) ||
          (B[K[7] = (g[w[K[5]][W[17][y]]] & K[6] ? 1 : 0) +
          (g[w[K[5]][W[17][y] + 1]] & K[6] ? 2 : 0) +
          (g[w[K[5]][W[17][y] + 2]] & K[6] ? 4 : 0) +
          (g[w[K[5]][W[17][y] + 3]] & K[6] ? 8 : 0) +
          (g[w[K[5]][W[17][y] + 4]] & K[6] ? 16 : 0) +
          ...
          (g[w[K[5]][W[17][y] + 6]] & K[6] ? 64 : 0) +
          (g[w[K[5]][W[17][y] + 7]] & K[6] ? 128 : 0)] == 1 &&
          g[K[8] = K[1] + (K[9] = w[K[5]][W[17][y] + b[K[7]] - 1])] & K[6]))
        {
          if (K[3] == K[9])
            continue;
          g[K[7]] &= ~K[6];
#if RJ > 2
          printf ("%d) M-Wing Type 1%c: %s SL %d @ %s %s SL %d @ %s %s => -%d @ %s\n",
            p, Y ? 'A' : 'B', S[K[0]], b[K[2]], S[K[3]], S[K[5]],
            b[K[6]], S[K[5]], S[K[9]], b[K[6]], S[K[7]]);
#endif
          if (solve (p))
            return 1;
#if RJ > 2
          printf ("%d) Undo M-Wing Type 1: %s SL %d @ %s %s SL %d @ %s %s <= +%d @ %s\n",
            p, Y ? 'A' : 'B', S[K[0]], b[K[2]], S[K[3]], S[K[5]],
            b[K[6]], S[K[5]], S[K[9]], b[K[6]], S[K[7]]);
#endif
          g[K[7]] |= K[6];
          ...
          prn ();
#endif
          return 0;          // Undo M-Wing Type 1A, 1B removal Cell values
        }
      }
    }
      ...
      {                      // Check 1st and 2nd Cell positions either Band or Stack wise
        ...
        {                    // (Type 2 01, 02) Check SL either Line or Box wise
          ...
          if (solve (p))
            return 1;
#if RJ > 2
          printf ("%d) Undo Grouped W-Wing Type 2 %s: %d @ %s %s SL %s %d between Cells %d @ r%dc%d and Cells %d @ r%dc%d\n<= +%d @ r%dc%d r%dc%d\n",
            p, A[2][0] ? "Stack" : "Band", b[g[K[0]]], S[K[0]], S[K[1]],
            K[9] ? (A[2][0] ? "Column" : "Row") : "Box",
            N ? (A[2][0] ? COL (w[L][20]) : ROW (w[L][20])) : BOX (L), b[K[9]],
            ROW (w[X][20] | w[w[X][W[3][A[2][0]]]][20] | w[w[X][W[4][A[2][0]]]][20]),
        ...
        {                    // Search W-Wing Type 1 SL Wings wise
            ...
            for (K[7] = 0; K[7] < 2; ++K[7])
              ...
              {              // (Type 1 01) Check SL Line wise
                ...
                if (solve (p))
                  return 1;
#if RJ > 2
                printf ("%d) Undo Grouped W-Wing Type 1 Grid: %d @ %s %s SL %s %d between Cells %d @ r%dc%d and Cell %d @ %s\n<= +%d @ %s %s\n",
                  p, b[g[K[0]]], S[K[0]], S[K[1]], K[6] ? "Column" : "Row",
                  K[6] ? COL (w[K[8]][20]) : ROW (w[K[8]][20]), b[N],
                  ROW (w[K[8]][20] | w[w[K[8]][W[3][K[6]]]][20] | w[w[K[8]][W[4][K[6]]]][20]),
                  COL (w[K[8]][20] | w[w[K[8]][W[3][K[6]]]][20] | w[w[K[8]][W[4][K[6]]]][20]),
                ...
                prn ();
#endif
                return 0;    // Undo W-Wing Type 1 removal Cells values
              }
          ...
          {                  // (Type 1 02) Check SL ERI wise
            ...
            printf ("%d) W-Wing Type 1 Grid: %d @ %s %s ERI %d @ b%d%s => -%d @ %s %s\n",
              p, b[g[K[0]]], S[K[0]], S[K[1]], b[N], BOX (K[3 + a]), S[K[3 + a]],
              b[K[2]], S[K[3]], S[K[4]]);
#endif
            if (solve (p))
              return 1;
          ...
          {                  // (Type 1 03, 04, Type 2 03, 04) Check SL either Line or ERI wise
            ...
            printf ("%s\n=> -%d @ r%dc%d r%dc%d\n", S[Z], b[K[2]],
              ...
              if (solve (p))
                return 1;
#if RJ > 2
            printf ("%d) Undo W-Wing Type %s: %d @ %s %s SL%s",
              p, A[2][0] != A[2][1] ? "1 Grid" : A[2][0] ? "2 Stack" : "2 Band", b[g[K[0]]],
              S[K[0]], S[K[1]], N ? (A[2][0] ? " Column " : " Row ") : " ");
            if (N)
              printf ("%d between Cell %d @ ", A[2][0] ? COL (w[X][20]) : ROW (w[X][20]), b[K[9]]);
            ...
            prn ();
#endif
            return 0;        // Undo W-Wing Type 1 and Type 2 removal Cells values
          }
        }
    }
  }
      ...
      {                      // Search 2nd Wing Cell position Column wise
        ...
        {                    // XY-Ring Type 1 removal Cell values
          ...
          printf ("%d) XY-Ring Type 1: %d @ r%dc%d r%dc%d => -%d @ r%dc%d => -%d @ r%dc%d => -%d @ r%dc%d => -%d @ r%dc%d\n",
            p, b[g[K[2]] | g[K[4]]], ROW (w[K[0]][20] | w[K[2]][20]), COL (w[K[0]][20] |
            w[K[2]][20]), ROW (w[K[4]][20] | w[K[6]][20]), COL (w[K[4]][20] | w[K[6]][20]),
            b[g[K[0]] & g[K[2]]], ROW (w[w[K[0]][6]][20] | w[w[K[0]][7]][20] |
            w[w[K[2]][6]][20] | w[w[K[2]][7]][20] | w[w[K[0]][K[5]]][20] |
            w[w[K[0]][K[5] + 1]][20] | w[w[K[0]][K[5] + 2]][20]), COL (w[w[K[0]][6]][20] |
            w[w[K[0]][7]][20] | w[w[K[2]][6]][20] | w[w[K[2]][7]][20] |
            w[w[K[0]][K[5]]][20] | w[w[K[0]][K[5] + 1]][20] | w[w[K[0]][K[5] + 2]][20]),
            b[g[K[4]] & g[K[6]]], ROW (w[w[K[4]][6]][20] | w[w[K[4]][7]][20] |
            w[w[K[6]][6]][20] | w[w[K[6]][7]][20] | w[w[K[4]][K[5]]][20] |
            w[w[K[4]][K[5] + 1]][20] | w[w[K[4]][K[5] + 2]][20]), COL (w[w[K[4]][6]][20] |
            w[w[K[4]][7]][20] | w[w[K[6]][6]][20] | w[w[K[6]][7]][20] |
            w[w[K[4]][K[5]]][20] | w[w[K[4]][K[5] + 1]][20] | w[w[K[4]][K[5] + 2]][20]),
            b[g[K[0]] & g[K[4]]], ROW (w[w[K[0]][12]][20] | w[w[K[0]][13]][20] |
            w[w[K[4]][12]][20] | w[w[K[4]][13]][20] | w[w[K[0]][K[7]]][20] |
            w[w[K[0]][K[7] + 1]][20] | w[w[K[0]][K[7] + 2]][20]), COL (w[w[K[0]][12]][20] |
            w[w[K[0]][13]][20] | w[w[K[4]][12]][20] | w[w[K[4]][13]][20] |
            w[w[K[0]][K[7]]][20] | w[w[K[0]][K[7] + 1]][20] | w[w[K[0]][K[7] + 2]][20]),
            b[g[K[2]] & g[K[6]]], ROW (w[w[K[2]][12]][20] | w[w[K[2]][13]][20] |
          ...
          if (solve (p))
            return 1;
#if RJ > 2
          printf ("%d) Undo XY-Ring Type 1: %d @ r%dc%d r%dc%d <= +%d @ r%dc%d => -%d @ r%dc%d => -%d @ r%dc%d => -%d @ r%dc%d\n",
            p, b[g[K[2]] | g[K[4]]], ROW (w[K[0]][20] | w[K[2]][20]), COL (w[K[0]][20] |
            w[K[2]][20]), ROW (w[K[4]][20] | w[K[6]][20]), COL (w[K[4]][20] | w[K[6]][20]),
            b[g[K[0]] & g[K[2]]], ROW (w[w[K[0]][6]][20] | w[w[K[0]][7]][20] |
            w[w[K[2]][6]][20] | w[w[K[2]][7]][20] | w[w[K[0]][K[5]]][20] |
        ...
        {                    // Check XY-Wing Type 1 removal Cell values
          ...
          printf ("%d) XY-Wing Type 1: %d @ r%dc%d %s => -%d @ %s\n",
            p, b[g[K[2]] | g[K[4]]], ROW (w[K[0]][20] | w[K[2]][20]),
            COL (w[K[0]][20] | w[K[2]][20]), S[K[4]], b[K[5]], S[K[6]]);
#endif
          if (solve (p))
            return 1;
#if RJ > 2
          printf ("%d) Undo XY-Wing Type 1: %d @ r%dc%d %s <= +%d @ %s\n",
            p, b[g[K[2]] | g[K[4]]], ROW (w[K[0]][20] | w[K[2]][20]),
            COL (w[K[0]][20] | w[K[2]][20]), S[K[4]], b[K[5]], S[K[6]]);
#endif
          g[K[6]] |= K[5];
          ...
          prn ();
#endif
          return 0;          // Undo Wing Cells common value to XY-Wing Type 1 removal Cell values
        }
        ...
        for (y = 0; y < 2; ++y)
        {
          if (B[g[K[0]]] < 3)
          {
            if ((g[L = w[K[2]][12 | y]] | g[w[L][6]] | g[w[L][7]]) &
              ERI (w[K[0]][12 | y]) & K[5])
            {                // ERI 47
              k[0] = L;
              k[1] = w[L][6];
            ...
            if ((g[L = w[K[4]][6 | y]] | g[w[L][12]] | g[w[L][13]]) &
              ERI (w[K[0]][6 | y]) & K[5])
            {                // ERI 48
              k[0] = L;
              k[1] = w[L][12];
          ...
          if ((g[L = w[K[2]][6 | y]] | (B[g[K[0]]] > 2 ? 0 :
            g[w[L][12]] | g[w[L][13]])) & ERI (w[K[6]][6 | y]) & K[5])
          {                  // ERI 49
            k[0] = L;
            if (B[g[K[0]]] < 3)
            {
          ...
          if ((g[L = w[K[4]][12 | y]] | (B[g[K[0]]] > 2 ? 0 :
            g[w[L][6]] | g[w[L][7]])) & ERI (w[K[6]][12 | y]) & K[5])
          {                  // ERI 50
            k[0] = L;
            if (B[g[K[0]]] < 3)
            {
          ...
          {                  // Box 53, Line 57, 69, 69a, Reduced Line Ring 61, 65
            ...
            else if (~(g[w[Z][6]] | g[w[Z][7]]) & (g[K[6]] | g[w[K[6]][13 ^ y]]) & K[5])
            {
              k[0] = w[K[6]][13 ^ y];
              k[4] = K[6];
            }
          ...
          {                  // Box 54, Line 58, 70, 70a, Reduced Line Ring 62, 66
            ...
            else if (~(g[w[Z][12]] | g[w[Z][13]]) & (g[K[6]] | g[w[K[6]][7 ^ y]]) & K[5])
            {
              k[0] = w[K[6]][7 ^ y];
              k[4] = K[6];
            }
...
XYWT1Tf:
            ...
            if (solve (p))
              return 1;
#if RJ > 2
            printf ("%d) Undo XY%sTransport:%s%s%s%d @ r%dc%d %s SL%s",
              p, B[g[K[0]]] < 3 ? "-Wing Type 1 " : "Z-", k[50] & 4 ? " Reduced " : " ",
              k[50] & 1 ? "Box" : (k[50] & 2 ? "ERI" : R_C), k[50] & 8 ? " Ring " : " ",
              b[g[K[2]] | g[K[4]]], ROW (w[K[0]][20] | w[K[2]][20]),
              COL (w[K[0]][20] | w[K[2]][20]), S[K[4]], A[1][0] + 1 ? " between " : " ");
            ...
            prn ();
#endif
            return 0;        // Undo XY-Wing Type 1 Transport and XYZ-Transport removal Cell values
          }
        }
        if (B[g[K[0]]] < 3 || !(g[K[6]] & K[5]))
          continue;          // Skip XYZ-Hybrid for either Apex Cell values < three digits; or Wing Cells common value not in Hybrid Cell values
        ...
        {                    // Wing Cells common value not in 1st Wing and Hybrid Cells Column other Cells values
                             // Backup and remove Wing Cells common value from XYZ-Hybrid Row wise removal Cells values
          k[0] = g[w[K[0]][6]];
          k[2] = g[w[K[0]][7]];
          k[3] = g[w[K[2]][6]];
          k[4] = g[w[K[2]][7]];
        ...
        {                    // Wing Cells common value not in 2nd Wing and Hybrid Cells Row other Cells values
                             // Backup and remove Wing Cells common value from XYZ-Hybrid Column wise removal Cells values
          k[1] = g[w[K[0]][12]];
          k[8] = g[w[K[0]][13]];
          k[9] = g[w[K[4]][12]];
          k[10] = g[w[K[4]][13]];
        ...
        if ((k[0] & k[1]) + 1)
        {
          ...
          prn ();
#endif
          return 0;        // Undo XYZ-Hybrid removal Cell values
        }
      }
    }
  }
  for (a = p; a < q; ++a)    // Search XY-Ring Type 2, XY-Wing Type 2, XY-Wing Type 2 Transport, XYZ-Wing, XYZ-Wing Transport and XYZ-Wing Hybrid Apex unsolved Cell positions wise
  {
    ...
    {
      if (!(g[w[K[0]][W[1][y]]] | g[w[K[0]][W[18][y]]] | g[w[K[0]][W[19][y]]] |
        ...
        {                    // Search 2nd Wing Cell position Box wise but not 1st Wing Line
          ...
          if (K[2] > K[0] && K[4] > K[0] &&
            ...
            {                // Search XY-Ring Type 2 3rd Wing Cell position 1st and 2nd Wing Cells common Cell positions wise
              ...
              if (B[g[K[5] = w[K[4]][Y]]] != 2 || B[g[K[2]] | g[K[5]]] != 3 ||
                ...
                (w[K[0]][20] & w[K[4]][20] & W[15][y] ? g[w[K[0]][W[1][!y]]] |
                g[w[K[0]][W[18][!y]]] | g[w[K[0]][W[19][!y]]] |
                g[w[K[0]][W[20][!y]]] | g[w[K[0]][W[21][!y]]] |
                g[w[K[0]][W[22][!y]]] : 0)) & g[K[0]] & g[K[4]]) ||
                ((g[w[K[2]][W[3][y]]] | g[w[K[2]][W[4][y]]] |
                g[w[K[5]][W[3][y]]] | g[w[K[5]][W[4][y]]] |
                g[Z = w[K[2]][K[8]]] | g[w[Z][W[3][y]]] | g[w[Z][W[4][y]]] |
                (w[K[2]][20] & w[K[5]][20] & W[15][y] ? g[w[K[2]][W[1][!y]]] |
                g[w[K[2]][W[18][!y]]] | g[w[K[2]][W[19][!y]]] |
                g[w[K[2]][W[20][!y]]] | g[w[K[2]][W[21][!y]]] |
                g[w[K[2]][W[22][!y]]] : 0)) & g[K[2]] & g[K[5]])))
                continue;    // no XY-Ring Type 2 removal Cell values
              int k[32] = {g[w[K[0]][W[3][y]]], g[w[K[0]][W[4][y]]],
                           ...
                           g[Z = w[K[2]][K[8]]], g[w[Z][W[3][y]]], g[w[Z][W[4][y]]],-1,-1};
                             // Backup XY-Ring Type 2 removal Cell values and drop Wing Cells common removal values
              g[w[K[0]][W[3][y]]] &= ~g[K[0]];
              g[w[K[0]][W[4][y]]] &= ~g[K[0]];
              g[w[K[2]][W[3][y]]] &= ~g[K[2]];
              g[w[K[2]][W[4][y]]] &= ~g[K[2]];
              ...
              g[w[K[4]][K[7] + 2]] &= ~(g[K[4]] & g[K[5]]);
              g[X] &= ~(g[K[0]] & g[K[4]]);
              g[w[X][W[3][y]]] &= ~(g[K[0]] & g[K[4]]);
              g[w[X][W[4][y]]] &= ~(g[K[0]] & g[K[4]]);
              g[Z] &= ~(g[K[2]] & g[K[5]]);
              ...
              {              // Check Apex Cell position and 2nd Wing Cell position same Line
                ...
                k[25] = g[w[K[0]][W[21][!y]]];
                k[26] = g[w[K[0]][W[22][!y]]];
                g[w[K[0]][W[1][!y]]] &= ~(g[K[0]] & g[K[4]]);
                g[w[K[0]][W[18][!y]]] &= ~(g[K[0]] & g[K[4]]);
                g[w[K[0]][W[19][!y]]] &= ~(g[K[0]] & g[K[4]]);
                g[w[K[0]][W[20][!y]]] &= ~(g[K[0]] & g[K[4]]);
                g[w[K[0]][W[21][!y]]] &= ~(g[K[0]] & g[K[4]]);
                g[w[K[0]][W[22][!y]]] &= ~(g[K[0]] & g[K[4]]);
              }
              ...
              {              // Check 1st Wing Cell position and 3rd Wing Cell position same Line
                ...
                k[30] = g[w[K[2]][W[21][!y]]];
                k[31] = g[w[K[2]][W[22][!y]]];
                g[w[K[2]][W[1][!y]]] &= ~(g[K[2]] & g[K[5]]);
                g[w[K[2]][W[18][!y]]] &= ~(g[K[2]] & g[K[5]]);
                g[w[K[2]][W[19][!y]]] &= ~(g[K[2]] & g[K[5]]);
                g[w[K[2]][W[20][!y]]] &= ~(g[K[2]] & g[K[5]]);
                g[w[K[2]][W[21][!y]]] &= ~(g[K[2]] & g[K[5]]);
                g[w[K[2]][W[22][!y]]] &= ~(g[K[2]] & g[K[5]]);
              }
#if RJ > 2
              printf ("%d) XY-Ring Type 2: %d @ r%dc%d r%dc%d => -%d @ r%dc%d => -%d @ r%dc%d\n=> -%d @ %s %s %s %s %s %s %s",
                p, b[g[K[2]] | g[K[4]]], ROW (w[K[0]][20] | w[K[2]][20]),
                COL (w[K[0]][20] | w[K[2]][20]), ROW (w[K[4]][20] | w[K[6]][20]),
                COL (w[K[4]][20] | w[K[6]][20]), b[g[K[0]] & g[K[2]]],
                ...
                w[w[K[2]][W[3][y]]][20] | w[w[K[2]][W[4][y]]][20] | w[w[K[0]][K[7]]][20] |
                w[w[K[0]][K[7] + 1]][20] | w[w[K[0]][K[7] + 2]][20]), b[g[K[4]] & g[K[5]]],
                ...
                w[w[K[5]][W[3][y]]][20] | w[w[K[5]][W[4][y]]][20] | w[w[K[4]][K[7]]][20] |
                w[w[K[4]][K[7] + 1]][20] | w[w[K[4]][K[7] + 2]][20]), b[g[K[0]] & g[K[4]]],
                S[w[K[0]][W[3][y]]], S[w[K[0]][W[4][y]]], S[w[K[4]][W[3][y]]],
                S[w[K[4]][W[4][y]]], S[X], S[w[X][W[3][y]]], S[w[X][W[4][y]]]);
              if (k[20] + 1)
                printf (" r%dc%d",
              ...
              printf ("\n=> -%d @ %s %s %s %s %s %s %s",
                ...
                S[w[K[5]][W[3][y]]], S[w[K[5]][W[4][y]]],
                S[Z], S[w[Z][W[3][y]]], S[w[Z][W[4][y]]]);
              if (k[21] + 1)
                printf (" r%dc%d",
              ...
              if (solve (p))
                return 1;
#if RJ > 2
              printf ("%d) Undo XY-Ring Type 2: %d @ r%dc%d r%dc%d\n<= +%d @ r%dc%d <= +%d @ r%dc%d\n<= +%d @ %s %s %s %s %s %s %s",
                p, b[g[K[2]] | g[K[4]]], ROW (w[K[0]][20] | w[K[2]][20]),
                COL (w[K[0]][20] | w[K[2]][20]), ROW (w[K[4]][20] | w[K[6]][20]),
                COL (w[K[4]][20] | w[K[6]][20]), b[g[K[0]] & g[K[2]]],
                ROW (w[w[K[0]][W[3][y]]][20] | w[w[K[0]][W[4][y]]][20] |
              ...
              prn ();
#endif
              return 0;      // Undo Wing Cells common value to XY-Ring Type 2 removal Cell values
            }
          if (B[g[K[0]] | g[K[2]] | g[K[4]]] > 3 ||
          ...
          {                  // Check Wing Cells common value in XY-Wing Type 2 and XYZ-Wing removal Cells values
            ...
            printf ("%d) XY%s: %d @ r%dc%d %s => -%d @ r%dc%d",
              p, B[g[K[0]]] < 3 ? "-Wing Type 2" : "Z-Wing", b[g[K[2]] | g[K[4]]],
            ...
            printf ("\n");
              ...
              if (solve (p))
                return 1;
#if RJ > 2
            printf ("%d) Undo XY%s: %d @ r%dc%d %s <= +%d @ r%dc%d",
              p, B[g[K[0]]] < 3 ? "-Wing Type 2" : "Z-Wing", b[g[K[2]] | g[K[4]]],
              ROW (w[K[0]][20] | w[K[2]][20]), COL (w[K[0]][20] | w[K[2]][20]),
              S[K[4]], b[K[5]], ROW (w[w[K[0]][W[3][y]]][20] | w[w[K[0]][W[4][y]]][20]),
              COL (w[w[K[0]][W[3][y]]][20] | w[w[K[0]][W[4][y]]][20]));
            ...
            g[w[K[0]][W[4][y]]] = k[1];
            if (B[g[K[0]]] < 3)
            {                // Undo Wing Cells common value to XY-Wing Type 2 more removal Cell positions
              g[w[K[4]][K[6]]] = k[2];
              g[w[K[4]][K[6] + 1]] = k[3];
              g[w[K[4]][K[6] + 2]] = k[4];
            }
            ...
            prn ();
#endif
            return 0;        // Undo XY-Wing Type 2 and XYZ-Wing removal Cells values
          }
          {                  // Search XY-Wing Type 2 Transport and XYZ-Wing Transport
            ...
            if (B[g[K[0]]] > 2)
            {
              ...
              {              // Line 01, Box 02
                k[0] = g[M]; // Backup and drop Wing Cells common value from XYZ-Wing Transport removal Cell positions
                k[1] = g[w[M][W[3][y]]];
                k[2] = g[w[M][W[4][y]]];
                g[M] &= ~K[5];
                g[w[M][W[3][y]]] &= ~K[5];
                ...
                prn ();
#endif
                return 0;    // Undo XYZ-Wing Transport removal Cell values
              }
            ...
            {                // Line Ring 05, Box Ring 06, Reduced Line Ring 07, 08, 09
              if (B[g[K[0]]] > 2)
              {
                k[0] = g[A];
                k[1] = g[w[A][W[3][y]]];
              ...
              g[w[M][W[3][y]]] &= Y;
              g[w[M][W[4][y]]] &= Y;
              k[15] = k[75] && (B[g[K[0]]] < 3 || w[K[0]][K[8]] == M) &&
                      ~(g[w[M][W[3][y]]] | g[w[M][W[4][y]]]) & K[5];
              k[16] = k[75] && ~(g[w[Y = w[K[2]][K[8]]][W[3][y]]] | g[w[Y][W[4][y]]]) & K[5];
              if (k[15])
              {              // Reduced Line Ring 07, 09
                k[17] = g[w[K[4]][W[1][!y]]];
                k[18] = g[w[K[4]][W[18][!y]]];
                k[19] = g[w[K[4]][W[19][!y]]];
                k[20] = g[w[K[4]][W[20][!y]]];
              ...
              if (B[g[K[0]]] > 2)
                printf (" %s %s %s", S[A], S[w[A][W[3][y]]], S[w[A][W[4][y]]]);
              if (k[15])
                printf (" r%dc%d",
              ...
              if (k[16])
                printf (" r%dc%d",
                  ...
                  w[w[K[2]][W[19][!y]]][20] | w[w[K[2]][W[20][!y]]][20] |
                  w[w[K[2]][W[21][!y]]][20] | w[w[K[2]][W[22][!y]]][20]));
              printf ("\n=> -%d @ r%dc%d => -%d @ %s %s %s %s %s %s %s\n",
                b[g[K[2]] ^ K[5]], ROW (w[w[K[0]][W[3][y]]][20] | w[w[K[0]][W[4][y]]][20] |
                w[w[K[2]][W[3][y]]][20] | w[w[K[2]][W[4][y]]][20] | w[L][20] |
                w[w[L][W[3][y]]][20] | w[w[L][W[4][y]]][20]), COL (w[w[K[0]][W[3][y]]][20] |
                w[w[K[0]][W[4][y]]][20] | w[w[K[2]][W[3][y]]][20] |
                w[w[K[2]][W[4][y]]][20] | w[L][20] | w[w[L][W[3][y]]][20] |
                w[w[L][W[4][y]]][20]), b[g[K[4]] ^ K[5]], S[w[K[0]][W[3][y]]],
                S[w[K[0]][W[4][y]]], S[w[K[4]][W[3][y]]], S[w[K[4]][W[4][y]]],
                S[M], S[w[M][W[3][y]]], S[w[M][W[4][y]]]);
#endif
              if (solve (p))
                return 1;
#if RJ > 2
            ...
            for (Y = W[1][!y]; Y < W[2][!y]; ++Y)
            {
              ...
              if (((k[75] = LN1 (w[K[2]][Y], w[K[4]][Y], w[Z][K[7]], y) & K[5]) ||
                ...
                g[M] | g[w[M][W[3][y]]] | g[w[M][W[4][y]]] |
                g[w[K[4]][W[3][y]]] | g[w[K[4]][W[4][y]]]) & (g[K[4]] ^ K[5]))))
              {              // Line Ring 10, 12, ERI Ring 11, 13
                k[0] = g[w[K[0]][W[3][y]]];
                k[1] = g[w[K[0]][W[4][y]]];
                k[2] = g[w[K[2]][W[3][y]]];
                k[3] = g[w[K[2]][W[4][y]]];
                ...
                g[w[K[0]][K[7]]] &= ~(g[K[2]] ^ K[5]);
                g[w[K[0]][K[7] + 1]] &= ~(g[K[2]] ^ K[5]);
                g[w[K[0]][K[7] + 2]] &= ~(g[K[2]] ^ K[5]);
                ...
                if (B[g[K[0]]] < 3 || w[K[0]][K[8]] == M)
                {            // Line Ring 12, ERI Ring 13
                  if (k[75])
                  {          // Line Ring 12
                    k[12] = g[w[X][W[3][!y]]];
                    k[13] = g[w[X][W[4][!y]]];
...
#if RJ > 2
                printf ("%d) XY%s Transport: %s Ring %d @ %s %s %s SL value %d %s %d between Cell %s and Cell %s\n=> -%d @ %s",
                  p, B[g[K[0]]] < 3 ? "-Wing Type 2" : "Z-Wing", k[75] ? R_C : "ERI",
                  b[g[K[2]] | g[K[4]]], S[K[0]], S[K[2]], S[K[4]], b[K[5]],
                ...
                if (B[g[K[0]]] < 3 || w[K[0]][K[8]] == M)
                {
                  if (k[75])
                    ...
                    printf (" %s %s %s %s %s", S[X], S[Z], S[w[Z][K[7]]], S[w[Z][K[7] + 1]], S[w[Z][K[7] + 2]]);
                  if (B[g[K[0]]] > 2)
                ...
                printf ("\n=> -%d @ r%dc%d => -%d @ %s %s %s %s %s %s %s\n",
                  b[g[K[2]] ^ K[5]], ROW (w[w[K[0]][W[3][y]]][20] |
                  w[w[K[0]][W[4][y]]][20] | w[w[K[2]][W[3][y]]][20] |
                  ...
                  if (solve (p))
                    return 1;
#if RJ > 2
                printf ("%d) Undo XY%s Transport: %s Ring %d @ %s %s %s SL value %d %s %d between Cell %s and Cell %s\n<= +%d @ %s",
                  p, B[g[K[0]]] < 3 ? "-Wing Type 2" : "Z-Wing", k[75] ? R_C : "ERI",
                  b[g[K[2]] | g[K[4]]], S[K[0]], S[K[2]], S[K[4]], b[K[5]],
                  k[75] ? R_C : "ERI", k[75] ? RCN (X) : BOX (X), S[X], S[Z], b[K[5]], S[M]);
                if (B[g[K[0]]] < 3 || w[K[0]][K[8]] == M)
              ...
              if (B[g[K[0]]] > 2 && w[K[0]][K[8]] != w[K[4]][N])
                for (A = 3; A < 5; ++A)
                  if (((k[75] = LN1 (w[L = w[w[K[4]][K[1]]][W[A][y]]][Y],
                    w[K[0]][Y], w[Z][K[7]], y) & K[5]) ||
                    (ERI (X) & ERI (Z) & K[5])) && g[L] & K[5])
                  {          // Line 14, ERI 15
                    g[L] -= K[5];
#if RJ > 2
                    printf ("%d) XYZ-Wing Transport: %s %d @ %s %s %s SL value %d %s %d between %s %s and %s %s\n=> -%d @ %s\n",
                      p, k[75] ? R_C : "ERI", b[g[K[0]]], S[K[0]], S[K[2]], S[K[4]], b[K[5]],
                      k[75] ? R_C : "ERI", k[75] ? RCN (X) : BOX (X), k[75] ? "Cell" : "ERI",
                      S[X], k[75] ? "Cell" : "ERI", S[Z], b[K[5]], S[L]);
#endif
                    if (solve (p))
                      return 1;
#if RJ > 2
                    printf ("%d) Undo XYZ-Wing Transport: %s %d @ %s %s %s SL value %d %s %d between %s %s and %s %s\n<= +%d @ %s\n",
                      p, k[75] ? R_C : "ERI", b[g[K[0]]], S[K[0]], S[K[2]], S[K[4]], b[K[5]],
                      k[75] ? R_C : "ERI", k[75] ? RCN (X) : BOX (X), k[75] ? "Cell" : "ERI",
                      S[X], k[75] ? "Cell" : "ERI", S[Z], b[K[5]], S[L]);
#endif
              ...
              for (M = 0; M < 2; ++M)
              {
                if (!M || B[g[K[0]]] < 3)
                  for (A = K[7]; A < K[7] + 3; ++A)
                    if (((k[75] = LN1 (w[L = w[K[W[16][M]]][A]][Y],
                      ...
                      (ERI (X) & ERI (Z) & K[5])) && ((g[L] | (!k[75] &&
                      (B[g[K[0]]] < 3 || w[K[0]][K[8]] == w[K[4]][N]) ?
                      g[w[K[2 << M]][Y]] : 0)) & K[5]))
                    {        // Line 16, ERI 17, 18
                      k[0] = g[L];
                      g[L] &= ~K[5];
                      if (!k[75] && (B[g[K[0]]] < 3 || w[K[0]][K[8]] == w[K[4]][N]))
                      {      // ERI 18
                        k[1] = g[w[K[2 << M]][Y]];
                        g[w[K[2 << M]][Y]] &= ~K[5];
                      }
#if RJ > 2
                      printf ("%d) XY%s Transport: %s %d @ %s %s %s SL value %d %s %d => -%d @ %s",
                        p, B[g[K[0]]] < 3 ? "-Wing Type 2" : "Z-Wing", k[75] ? R_C : "ERI",
                        b[g[K[2]] | g[K[4]]], S[K[0]], S[K[2]], S[K[4]], b[K[5]],
                      ...
                      if (solve (p))
                        return 1;
#if RJ > 2
                      printf ("%d) Undo XY%s Transport: %s %d @ %s %s %s SL value %d %s %d <= +%d @ %s",
                        p, B[g[K[0]]] < 3 ? "-Wing Type 2" : "Z-Wing", k[75] ? R_C : "ERI",
                        b[g[K[2]] | g[K[4]]], S[K[0]], S[K[2]], S[K[4]], b[K[5]],
                        k[75] ? R_C : "ERI Box", k[75] ? RCN (X) : BOX (X), b[K[5]], S[L]);
                      if (k[1] + 1)
...
#endif
                      return 0;
                    }        // Undo XY-Wing Type 2 Transport and XYZ-Wing Transport removal Cell values
                for (A = 3; A < 5; ++A)
                  ...
                  {          // Line 19, 21, 23, 25 ERI 20, 22, 24, 26
                    k[0] = g[L];
                    g[L] &= ~K[5];
                    if (B[g[K[0]]] < 3 || w[K[0]][K[8]] == w[K[4]][N])
                    {        // Line 19, 23 ERI 20, 23
                      if (M)
                      {      // Line 23, ERI 24
                        k[1] = g[w[L][N]];
                        g[w[L][N]] &= ~K[5];
...
#if RJ > 2
                    printf ("%d) XY%s Transport: %s %d @ %s %s %s SL value %d %s %d between Cell %s and Cell %s\n=> -%d @ %s",
                      p, B[g[K[0]]] < 3 ? "-Wing Type 2" : "Z-Wing",
                      k[75] ? R_C : "ERI", b[g[K[2]] | g[K[4]]],
                      S[K[0]], S[K[2]], S[K[4]], b[K[5]], k[75] ? R_C : "ERI",
                      k[75] ? RCN (X) : BOX (X), S[X], S[Z], b[K[5]], S[L]);
                    if (k[1] + 1)
                    {
                    ...
                    if (solve (p))
                      return 1;
#if RJ > 2
                    printf ("%d) Undo XY%s Transport: %s %d @ %s %s %s SL value %d %s %d between Cell %s and Cell %s\n<= +%d @ %s",
                      p, B[g[K[0]]] < 3 ? "-Wing Type 2" : "Z-Wing",
                      k[75] ? R_C : "ERI", b[g[K[2]] | g[K[4]]],
                      S[K[0]], S[K[2]], S[K[4]], b[K[5]], k[75] ? R_C : "ERI",
                      k[75] ? RCN (X) : BOX (X), S[X], S[Z], b[K[5]], S[L]);
                ...
                {            // ERI 29, 30
                  g[w[K[4 >> M]][Y]] -= K[5];
#if RJ > 2
                  printf ("%d) XY%s Transport: ERI %d @ %s %s %s SL ERI %d @ b%d%s => -%d @ %s\n",
                    p, B[g[K[0]]] < 3 ? "-Wing Type 2" : "Z-Wing",
                    b[g[K[2]] | g[K[4]]], S[K[0]], S[K[2]], S[K[4]], b[K[5]],
                  ...
                  if (solve (p))
                    return 1;
#if RJ > 2
                  printf ("%d) Undo XY%s Transport: ERI %d @ %s %s %s SL ERI %d @ b%d%s <= +%d @ %s\n",
                    p, B[g[K[0]]] < 3 ? "-Wing Type 2" : "Z-Wing",
                    b[g[K[2]] | g[K[4]]], S[K[0]], S[K[2]], S[K[4]], b[K[5]],
                    BOX (w[K[2 << M]][Y]), S[w[K[2 << M]][Y]], b[K[5]], S[w[K[4 >> M]][Y]]);
#endif
  ...
  for (a = p; a < q; ++a)    // Search WXYZ-Wing Type 1, Almost Locked Set move Type 1a and Type 1b Apex unsolved Cell positions wise
  {
    if (B[g[r[a]]] > 4 ||    // Skip for either unsolved Cell values > four digits; or
      !(g[w[r[a]][0]] | g[w[r[a]][1]] | g[w[r[a]][2]] |
      g[w[r[a]][3]] | g[w[r[a]][4]] | g[w[r[a]][5]]) ||
      !(g[w[r[a]][14]] | g[w[r[a]][15]] | g[w[r[a]][16]] |
      g[w[r[a]][17]] | g[w[r[a]][18]] | g[w[r[a]][19]]))
          ...
          {                  // Search 3rd Wing Cell position
            ...
            {                // Check no Wing Cells common value in WXYZ-Wing Type 1 removal Cell values
              ...
              if (solve (p))
                return 1;
#if RJ > 2
              printf ("%d) Undo WXYZ-Wing Type 1: %d @ r%dc%d %s <= +%d @ ",
                p, b[g[K[2]] | g[K[4]] | g[K[6]]], 
                ROW (w[K[0]][20] | w[K[2 << Y]][20] | w[K[6]][20]),
                COL (w[K[0]][20] | w[K[2 << Y]][20] | w[K[6]][20]), S[K[4 >> Y]], b[K[7]]);
              if (k[5] + 1)
              ...
              prn ();
#endif
              return 0;      // Undo Wing Cells common value to WXYZ-Wing Type 1 removal Cell values
            }
// WXYZ-Wing Type 1 Transport
        ...
        {                    // Almost Locked Set move Type 1a removal Cell values
          int k[7] = {g[w[K[0]][W[3][Y]]], g[w[K[0]][W[4][Y]]],
                      ...
                      g[w[K[0]][Z]], g[w[K[0]][Z + 1]], g[w[K[0]][Z + 2]]};
                             // Backup Almost Locked Set move Type 1a removal Cell values and drop Wing Cells common removal values
          g[w[K[0]][W[3][Y]]] &= ~K[6];
          g[w[K[0]][W[4][Y]]] &= ~K[6];
          g[w[y][W[3][Y]]] &= ~K[6];
          g[w[y][W[4][Y]]] &= ~K[6];
          ...
          if (solve (p))
            return 1;
#if RJ > 2
          printf ("%d) Undo Almost Locked Set move Type 1a: %d @ r%dc%d r%dc%d <= +%d r%dc%d\n",
            p, b[g[K[2]] | g[K[4]]], ROW (w[K[0]][20] | w[K[2]][20]), COL (w[K[0]][20] |
            w[K[2]][20]), ROW (w[K[4]][20] | w[K[5]][20]), COL (w[K[4]][20] | w[K[5]][20]),
            b[K[6]], ROW (w[w[K[0]][W[3][Y]]][20] | w[w[K[0]][W[4][Y]]][20] |
            w[w[y][W[3][Y]]][20] | w[w[y][W[4][Y]]][20] | w[w[K[0]][Z]][20] |
        ...
        {                    // Search Almost Locked Set move Type 1b 3rd Wing Cell position Line wise
          if (B[g[K[0]] | g[K[2 << Y]]] != 3 || B[g[K[0]] & g[K[4 >> Y]]] != 1)
            continue;        // Skip for either Apex and 1st (or 2nd) Wing Cells values not three digits; or one common digit not in Apex and 2nd (or 1st) Wing Cells values
          ...
          {                  // Search 3rd Wing Cell position 1st (or 2nd) Wing Box+Line wise
            if (w[K[6] = w[K[4 >> Y]][K[5]]][20] & w[K[2 << Y]][20] & W[15][Y] ||
              ...
              B[g[K[6]]] != 2 || B[g[K[0]] | g[K[6]]] != 4 ||
                             // 3rd Wing Cell values not two digits; or Apex and 3rd Wing Cells values not four digits; or
              B[g[K[2]] & g[K[6]]] != 1 || B[g[K[4]] & g[K[6]]] != 1)
              continue;      // one common digit not in 1st and 3rd Wing Cells values; or one common digit not in 2nd and 3rd Wing Cells values
            ...
            if ((k[6] | k[7] | k[8] | k[9] | k[10] | k[11]) & K[7])
            {
              ...
              prn ();
#endif
              return 0;      // Undo Almost Locked Set move Type 1b removal Cell values
            }
          }
        }
      }
          ...
          for (Y = 0; Y < 2; ++Y)
            ...
            {                // Search 3rd Wing Cell position
              ...
              {              // Check WXYZ-Wing Type 2a, Type 2b and Type 3 removal Cells values
                ...
                prn ();
#endif
                return 0;    // Undo WXYZ-Wing Type 2a, Type 2b and Type 3 removal Cells values
              }
...
// WXYZ-Wing Type 2a, Type 2b and Type 3 Transport
          ...
          {                  // Search 2nd Apex Cell position mini-Line wise
            ...
            int k[4] = {g[w[K[0]][W[7 - K[5]][y]]]};
                             // Backup WXYZ-Wing Type 4b removal Cell position
            Z = -1;
            if (~(g[K[0]] | g[K[6]]) & K[7])
            {                // Check no Wing Cells common value in Apex Cells values
              ...
              k[2] = g[w[K[2]][Z + 1]];
              k[3] = g[w[K[2]][Z + 2]];
            }                // Backup WXYZ-Wing Type 4a more removal Cell positions
          ...
          if (B[g[K[0]]] == 3 && B[g[K[0]] | g[K[2]]] == 4 &&
            ...
            B[g[K[0]] | g[K[4]]] == 4 && B[g[K[2]] & g[K[4]]] == 1)
            ...
            {                // Search Almost Locked Set move Type 2c 3rd Wing Cell position 1st Wing Box and 2nd Wing Line wise
              ...
              if ((k[0] | k[1] | k[2] | k[3]) & (K[6] = g[K[0]] & g[K[5]]))
              {
                if (B[g[K[2]]] < 3)
                {
                  g[w[K[4]][W[3][y]]] &= ~K[6];
                ...
                printf ("%d) Almost Locked Set move Type 2c: %d @ r%dc%d r%dc%d => -%d @",
                  p, b[g[K[0]] | g[K[5]]], ROW (w[K[0]][20] | w[K[4]][20]),
                ...
                if (solve (p))
                  return 1;
#if RJ > 2
                printf ("%d) Undo Almost Locked Set move Type 2c: %d @ r%dc%d r%dc%d <= +%d @",
                  p, b[g[K[0]] | g[K[5]]], ROW (w[K[0]][20] | w[K[4]][20]),
                  COL (w[K[0]][20] | w[K[4]][20]), ROW (w[K[2]][20] | w[K[5]][20]),
                  COL (w[K[2]][20] | w[K[5]][20]), b[K[6]]);
                if (B[g[K[2]]] < 3)
          ...
          if (B[g[K[0]]] > 2)// Check Almost Locked Set move Type 2a for Apex Cell values three digits
            ...
            {                // Search Almost Locked Set move Type 2a 3rd Wing Cell position 1st Wing Box and 2nd Wing Line wise
              if (B[g[K[5] = w[K[2]][Y]]] != 2 || B[g[K[0]] | g[K[5]]] != 4 ||
                             // Check for 3rd Wing Cell values not two digits; or Apex and 3rd Wing Cells values not four digits; or
                B[g[K[2]] | g[K[5]]] != 3 || B[g[K[4]] | g[K[5]]] != 3)
                continue;    // 1st and 3rd Wing Cells values not three digits; or 2nd and 3rd Wing Cells values not three digits; or
              int X,
                  L;
              ...
              X = B[g[K[0]] & g[K[2]]] == 2;
              if (((g[Z = w[K[0]][K[6 | X]]] | g[w[Z][W[3][y]]] | g[w[Z][W[4][y]]] |
                g[w[K[0]][W[3][y]]] | g[w[K[0]][W[4][y]]]) & (L = ~g[K[5]] & g[K[4 >> X]])) ||
                ((g[w[K[4 >> X]][W[3][y]]] | g[w[K[4 >> X]][W[4][y]]]) & g[K[4 >> X]]))
              {
                int k[13] = {g[w[K[4 >> X]][W[3][y]]], g[w[K[4 >> X]][W[4][y]]],
                           g[Z], g[w[Z][W[3][y]]], g[w[Z][W[4][y]]],
                           g[w[K[0]][W[3][y]]], g[w[K[0]][W[4][y]]],-1};

                g[w[K[4 >> X]][W[3][y]]] &= ~g[K[4 >> X]];
                g[w[K[4 >> X]][W[4][y]]] &= ~g[K[4 >> X]];
                g[Z] &= ~L;
                g[w[Z][W[3][y]]] &= ~L;
                g[w[Z][W[4][y]]] &= ~L;
                g[w[K[0]][W[3][y]]] &= ~L;
                g[w[K[0]][W[4][y]]] &= ~L;
                if (X && (w[K[0]][20] & w[K[2]][20] & W[15][y]))
...
#if RJ > 2
                printf ("%d) Almost Locked Set move Type 2a: %d @ r%dc%d r%dc%d => -%d @ r%dc%d -%d @ ",
                  p, b[g[K[2]] | g[K[4]]], ROW (w[K[0]][20] | w[K[4]][20]),
                  COL (w[K[0]][20] | w[K[4]][20]), ROW (w[K[2]][20] | w[K[5]][20]),
                  COL (w[K[2]][20] | w[K[5]][20]), b[g[K[4 >> X]]],
                  ROW (w[w[K[4 >> X]][W[3][y]]][20] | w[w[K[4 >> X]][W[4][y]]][20]),
                  COL (w[w[K[4 >> X]][W[3][y]]][20] | w[w[K[4 >> X]][W[4][y]]][20]), b[L]);
                if (X)
                {
            ...
            if (solve (p))
              return 1;
#if RJ > 2
                printf ("%d) Undo Almost Locked Set move Type 2a: %d @ r%dc%d r%dc%d <= +%d @ r%dc%d -%d @ ",
                  p, b[g[K[2]] | g[K[4]]], ROW (w[K[0]][20] | w[K[4]][20]),
                  COL (w[K[0]][20] | w[K[4]][20]), ROW (w[K[2]][20] | w[K[5]][20]),
                  COL (w[K[2]][20] | w[K[5]][20]), b[g[K[4 >> X]]],
                  ROW (w[w[K[4 >> X]][W[3][y]]][20] | w[w[K[4 >> X]][W[4][y]]][20]),
                ...
                prn ();
#endif
                return 0;    // Undo Almost Locked Set move Type 2a removal Cells values
              }
            }
          if (B[g[K[0]] & g[K[2]]] != 1 || B[g[K[0]] | g[K[4]]] != 3)
          ...
          {                  // Search Almost Locked Set move Type 2b 3rd Wing Cell position 2nd Wing Line but not Box wise
            if (B[g[K[5] = w[K[2]][Y]]] != 2 || B[g[K[0]] | g[K[5]]] != 4 ||
            ...
            if ((g[K[6] = K[4 | y] - COL (w[K[4 | y]][20]) + COL (w[K[5 - y]][20])] |
              g[w[K[6]][W[3][y]]] | g[w[K[6]][W[4][y]]] | (B[g[K[0]]] > 2 ? 0 :
              g[K[7] = K[5 - y] - COL (w[K[5 - y]][20]) + COL (w[K[4 | y]][20])] |
              g[w[K[7]][W[3][y]]] | g[w[K[7]][W[4][y]]])) & g[K[4]] & g[K[5]])
            {
              int k[7] = {g[K[4]] & g[K[5]], g[K[6]], g[w[K[6]][W[3][y]]],
                         g[w[K[6]][W[4][y]]]};
              ...
              if (solve (p))
                return 1;
#if RJ > 2
              printf ("%d) Undo Almost Locked Set move Type 2b: %d @ r%dc%d r%dc%d <= +%d @ r%dc%d",
                p, b[g[K[2]] | g[K[4]]], ROW (w[K[0]][20] | w[K[4]][20]),
                COL (w[K[0]][20] | w[K[4]][20]), ROW (w[K[2]][20] | w[K[5]][20]),
                COL (w[K[2]][20] | w[K[5]][20]), b[k[0]],
                ROW (w[K[6]][20] | w[w[K[6]][W[3][y]]][20] | w[w[K[6]][W[4][y]]][20]),
              ...
              prn ();
#endif
              return 0;      // Undo Almost Locked Set move Type 2b removal Cells values
            }
          }
        }
      }
    ...
    if (Z + 1)               // One Trivalue Cell position found
    {
      for (Y = g[Z]; y = Y & -Y; Y -= y)
        ...
        {                    // Digit found Row, Box or Column wise in two Cell positions
          int k = g[Z];      // Backup BUG+1 Trivalue removal Cell values

          g[Z] = y;          // Remove other digits from Trivalue removal Cell values
#if RJ > 2
          ...
          prn ();
#endif
          return 0;          // Undo BUG+1 removal Cell values
        }
    }
#if RJ > 2
    else
  ...
  if (++n[3] > n[4])
    ++n[4];
NHSCF:
  if (x > p)                 // Check current Cell position for sorting and removing
  {
    a = r[p];
    r[p] = r[x];
    r[x] = a;
...
#endif
    if (p + 1 == q || solve (p + 1))
      return 1;              // Check either puzzle solved; or recursive solve for next unsolved Cell position
#if RJ > 2
