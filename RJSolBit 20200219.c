...
static const int b[512] = {  // Bitwise to digit Cell values
  ...
  { 4, 5, 7, 8, 0, 1, 2, 3, 6}, { 4, 6, 7, 8, 0, 1, 2, 3, 5}, { 5, 6, 7, 8, 0, 1, 2, 3, 4}},
           j[54][15] = {     // 3 Locked value Cell positions and 6 Line + 6 Box Cell peer positions for 54 mini-Lines
...
int solve (int p)
{
  ...
  for (; y < 27; ++y)        // Search Naked/Hidden Tuples Cell values Unit wise
  {
    ...
    if (Y > 6)
      continue;              // Skip Unit for solved Cell positions > 6
    ...
    if (Y > 4)
      continue;              // Skip Unit for solved Cell positions > 4
    ...
    if (Y > 2)
      continue;              // Skip Unit for solved Cell positions > 2
  ...
  for (a = 0; a < 54; ++a)   // Search Locked value for 54 mini-Lines 3 Cell positions
  ...
  for (a = 0; a < 54; ++a)   // Search Almost Locked Pair and Triple 54 mini-Lines 3 Cell positions wise
  {
    ...
    for (y = 3; y < 9; ++y)  // Search Almost Locked Pair and Triple 54 mini-Lines away Line 6 Cell positions wise
    {
      ...
      {                      // Found Almost Locked Pair Line wise
        ...
        printf ("%d) Almost Locked Pair: %s %d Box %d wise %d @ r%dc%d %s b%d%s\n=> -%d @ %s %s %s %s %s",
          p, a < 27 ? "Row" : "Column", a < 27 ? ROW (w[j[a][y]][20]) : COL (w[j[a][y]][20]),
          BOX (j[a][Z]), b[Y], ROW (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]),
          COL (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]), S[j[a][y]],
          BOX (j[a][Z]), S[j[a][Z]], b[Y], S[j[a][9 + (Z < 10)]], S[j[a][10 + (Z < 11)]],
          S[j[a][11 + (Z < 12)]], S[j[a][12 + (Z < 13)]], S[j[a][13 + (Z < 14)]]);
        if (k[0] ^ Y)
          printf (" => -%d @ %s", b[k[0] ^ Y], S[j[a][y]]);
        printf ("\n");
#endif
        if (solve (p))
          return 1;
      ...
      for (L = 3; L < 8; ++L)
      {                      // Search 1st Cell position away Line wise
        if (!g[j[a][L]])
        ...
        {                    // Search 2nd Cell position away Line wise
          ...
          {                  // Search 3rd Cell position away Box wise
            ...
            {                // Search 4th Cell position away Box wise
              ...
              g[j[a][L]] = k[0];
              g[j[a][M]] = k[1];
              g[j[a][9 + (N < 10) + (X < 11)]] = k[2];
              g[j[a][10 + (N < 11) + (X < 12)]] = k[3];
              g[j[a][11 + (N < 12) + (X < 13)]] = k[4];
              g[j[a][12 + (N < 13) + (X < 14)]] = k[5];
#if RJ > 3
              prn ();
#endif
    ...
    for (; y < 15; ++y)  // Search Almost Locked Pair and Triple 54 mini-Lines away Box 6 Cell positions wise
    {
      if (B[Y = g[j[a][y]] & ~(g[j[a][9 + (y < 10)]] |
        g[j[a][10 + (y < 11)]] | g[j[a][11 + (y < 12)]] |
        g[j[a][12 + (y < 13)]] | g[j[a][13 + (y < 14)]])] == 2 &&
        (g[j[a][Z = 3]] == Y || g[j[a][Z = 4]] == Y ||
        g[j[a][Z = 5]] == Y || g[j[a][Z = 6]] == Y ||
        g[j[a][Z = 7]] == Y || g[j[a][Z = 8]] == Y) &&
        (B[g[j[a][y]]] > 2 || (g[j[a][3 + (Z < 4)]] |
        g[j[a][4 + (Z < 5)]] | g[j[a][5 + (Z < 6)]] |
        g[j[a][6 + (Z < 7)]] | g[j[a][7 + (Z < 8)]]) & Y))
      {                      // Found Almost Locked Pair Box wise
        int k[7] = {g[j[a][y]], g[j[a][3]], g[j[a][4]], g[j[a][5]],
        ...
        printf ("%d) Almost Locked Pair: Box %d %s %d wise %d @ r%dc%d b%d%s %s\n=> -%d @ %s %s %s %s %s",
          p, BOX (j[a][y]), a < 27 ? "Row" : "Column",
          a < 27 ? ROW (w[j[a][Z]][20]) : COL (w[j[a][Z]][20]), b[Y],
          ROW (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]),
          COL (w[j[a][0]][20] | w[j[a][1]][20] | w[j[a][2]][20]), BOX (j[a][y]),
          S[j[a][y]], S[j[a][Z]], b[Y], S[j[a][3 + (Z < 4)]], S[j[a][4 + (Z < 5)]],
          S[j[a][5 + (Z < 6)]], S[j[a][6 + (Z < 7)]], S[j[a][7 + (Z < 8)]]);
        if (k[0] ^ Y)
          printf (" => -%d @ %s", b[k[0] ^ Y], S[j[a][y]]);
        printf ("\n");
#endif
      ...
      for (L = 9; L < 14; ++L)
      {                      // Search 1st Cell position away Box wise
        if (!g[j[a][L]])
          continue;          // Skip for 1st Cell position not unsolved
        for (M = L + 1; M < 15; ++M)
          ...
          {                  // Search 3rd Cell position away Line wise
            ...
            {                // Search 4th Cell position away Line wise
              ...
              printf ("%d) Almost Locked Triple: Box %d %s %d wise %d @ r%dc%d b%d%s b%d%s r%dc%d\n=> -%d @ %s %s %s %s",
                ...
                ROW (w[j[a][N]][20] | w[j[a][X]][20]),
                COL (w[j[a][N]][20] | w[j[a][X]][20]),
                b[Y], S[j[a][3 + (N < 4) + (X < 5)]],
                S[j[a][4 + (N < 5) + (X < 6)]], S[j[a][5 + (N < 6) + (X < 7)]],
                S[j[a][6 + (N < 7) + (X < 8)]]);
              if (k[0] ^ (k[0] & Y))
                printf (" => -%d @ %s",
                  b[k[0] ^ (k[0] & Y)], S[j[a][L]]);
  ...
  for (a = 0; a < 81; ++a)   // Search 2-String Kite and Grouped 2-String Kite with ERI Ring, ERI and Box Grid wise
  {
    ...
    {                        // Search 2-String Kite and Grouped 2-String Kite with ERI Ring and ERI digit wise
      ...
      if (solve (p))
        return 1;
#if RJ > 2
      printf ("%d) Undo%s2-String Kite%s SL value %d Row %d between Cell %d @ ", p,
        ((g[w[a][6]] & g[w[a][7]]) | (g[w[a][12]] & g[w[a][13]])) & Y ? " Grouped " : " ",
        K[0] + 1 ? "+ERI Ring:" : (K[1] + 1 ? "+ERI:" : (K[2] + 1 ? "+Box:" : ":")),
        ROW (w[a][20]), b[Y]);
      if (g[w[a][6]] & g[w[a][7]] & Y)
...
XYWT2Tf:
            ...
            if (k[6])
            {
              ...
              k[64] = g[k[50] = w[K[0]][K[7] + 1]];
              k[65] = g[k[51] = w[K[0]][K[7] + 2]];
...
#if RJ > 2
            printf ("%d) XY%s Transport: %s%s%d @ r%dc%d %s\nSL %s %d between %d @ %s and",
              p, B[g[K[0]]] < 3 ? "-Wing Type 2" : "Z-Wing",
              k[75] > 2 ? "ERI" : (k[75] > 1 ? "Box" : R_C), k[6] ? " Ring " : " ",
              b[g[K[2]] | g[K[4]]], ROW (w[K[0]][20] | w[K[2]][20]),
              COL (w[K[0]][20] | w[K[2]][20]), S[K[4]], k[75] > 2 ? "ERI Box" : R_C,
              k[75] > 2 ? BOX (X) : RCN (X), b[K[5]], S[X]);
            if (k[75] > 2)
              printf (" ERI %d @ b%d", b[K[5]], BOX (Z));
            else
              printf (" Cell %d @ ", b[K[5]]);
            printf ("%s => -%d @", S[Z], b[K[5]]);
            if (k[0] + 1)
            {
              printf (" %s", S[k[0]]);
              if (k[1] + 1)
            ...
            if (k[4] + 1)
              printf (" r%dc%d",
                ROW (w[k[4]][20] | w[k[7]][20] | w[k[8]][20] | w[k[9]][20] |
                w[k[10]][20] | w[k[11]][20]), COL (w[k[4]][20] | w[k[7]][20] |
                w[k[8]][20] | w[k[9]][20] | w[k[10]][20] | w[k[11]][20]));
            if (k[5] + 1)
              printf (" r%dc%d",
                ROW (w[k[5]][20] | w[k[12]][20] | w[k[13]][20] | w[k[14]][20] |
                w[k[15]][20] | w[k[16]][20]), COL (w[k[5]][20] | w[k[12]][20] |
                w[k[13]][20] | w[k[14]][20] | w[k[15]][20] | w[k[16]][20]));
            printf ("\n");
            if (k[6])
              printf ("=> -%d @ r%dc%d => -%d @ r%dc%d\n", b[g[K[2]] ^ K[5]],
                ROW (w[k[47]][20] | w[k[48]][20] | w[k[49]][20] | w[k[50]][20] |
                w[k[51]][20] | w[k[52]][20] | w[k[53]][20]), COL (w[k[47]][20] |
                w[k[48]][20] | w[k[49]][20] | w[k[50]][20] | w[k[51]][20] |
                w[k[52]][20] | w[k[53]][20]), b[g[K[4]] ^ K[5]], ROW (w[k[54]][20] |
                w[k[55]][20] | w[k[56]][20] | w[k[57]][20] | w[k[58]][20] |
                w[k[59]][20] | w[k[60]][20]), COL (w[k[54]][20] | w[k[55]][20] |
                w[k[56]][20] | w[k[57]][20] | w[k[58]][20] | w[k[59]][20] | w[k[60]][20]));
#endif
            if (solve (p))
              return 1;
#if RJ > 2
            printf ("%d) Undo XY%s Transport: %s%s%d @ r%dc%d %s\nSL %s %d between %d @ %s and",
              p, B[g[K[0]]] < 3 ? "-Wing Type 2" : "Z-Wing",
              k[75] > 2 ? "ERI" : (k[75] > 1 ? "Box" : R_C), k[6] ? " Ring " : " ",
              b[g[K[2]] | g[K[4]]], ROW (w[K[0]][20] | w[K[2]][20]),
              COL (w[K[0]][20] | w[K[2]][20]), S[K[4]], k[75] > 2 ? "ERI Box" : R_C,
            ...
            prn ();
#endif
            return 0;        // Undo XY-Wing Type 2 Transport removal Cell values
          }
TEMP:
*/
          {                  // Search XY-Wing Type 2 Transport and XYZ-Wing Transport
            ...
            if (((k[75] = LB3 (M = w[K[4]][N], w[X][K[1]], w[X][K[7]], y) & K[5]) ||
              ...
              g[k[76] = w[K[4]][K[1]]] | g[w[k[76]][W[3][y]]] | g[w[k[76]][W[4][y]]]) |
            ...
            {                // Line Ring 05, Box Ring 06, Reduced Line Ring 07, 08, 09
              ...
              if (B[g[K[0]]] > 2)
                printf (" %s %s %s", S[k[76]], S[w[k[76]][W[3][y]]], S[w[k[76]][W[4][y]]]);
              if (k[15])
                printf (" r%dc%d",
            ...
            for (Y = W[1][!y]; Y < W[2][!y]; ++Y)
            {
              ...
              if (((k[75] = LN1 (w[K[2]][Y], w[K[4]][Y], w[Z][K[7]], y) & K[5]) ||
                ...
                g[w[K[4]][K[1]]] | g[w[K[2]][K[8]]] | g[L = w[K[2]][k[76]]] |
                g[w[L][W[3][!y]]] | g[w[L][W[4][!y]]] | g[L = w[K[4]][k[76]]] |
              ...
              {              // Line Ring 10, 12, ERI Ring 11, 13
                ...
                if (B[g[K[0]]] < 3 || w[K[0]][K[8]] == M)
                {
                  ...
                  printf (" %s %s %s %s %s %s %s", S[w[K[2]][K[8]]],
                    S[w[K[2]][k[76]]], S[w[w[K[2]][k[76]]][W[3][!y]]], S[w[w[K[2]][k[76]]][W[4][!y]]],
                    S[w[K[4]][k[76]]], S[w[w[K[4]][k[76]]][W[3][!y]]], S[w[w[K[4]][k[76]]][W[4][!y]]]);
                }
                printf ("\n=> -%d @ r%dc%d => -%d @ %s %s %s %s %s %s %s\n",
              ...
              if (B[g[K[0]]] > 2 && w[K[0]][K[8]] != w[K[4]][N])
                for (k[76] = 3; k[76] < 5; ++k[76])
                  if (((k[75] = LN1 (w[L = w[w[K[4]][K[1]]][W[k[76]][y]]][Y],
              ...
              {
                if (!M || B[g[K[0]]] < 3)
                  for (k[76] = K[7]; k[76] < K[7] + 3; ++k[76])
                    if (((k[75] = LN1 (w[L = w[K[W[16][M]]][k[76]]][Y],
                ...
                for (k[76] = 3; k[76] < 5; ++k[76])
                  if (((k[75] = LN1 (w[L = w[K[2 << M]][W[k[76]][y]]][Y],
